"""Alembic migration environment for the Planning Service.

Configures an async SQLAlchemy engine so that Alembic uses the
same async driver (``asyncpg``) as the rest of the service.  The
database URL is read from the ``.env`` file via ``python-dotenv``
and overrides the placeholder value in ``alembic.ini``.

Only online mode is supported; offline (SQL-script) mode is not
implemented because all migrations are applied directly against
...
# Override placeholder URL in alembic.ini with value from .env.
_DATABASE_URL: str = os.getenv("PLANNING_DATABASE_URL", "")
...
async def _run_async_migrations() -> None:
    ...
    Opens a single async connection, delegates to
    ``_do_run_migrations`` via ``run_sync``, then disposes the
    engine cleanly.
    """
